---
layout: post
title: "Lerping Every Frame: What Curve Is It?"
description: "What curve do we end up with when iteratively lerping a value over multiple frames? An exponential!"
---
...
Iteratively lerping from 0 to 1 with a factor $$d$$ given a frame number $$x$$ can be expressed as:
...
[^2]: Making $$d$$ a multiple of time ensures lerp does not only rely on how many frames have passed. Time elapsed and frames rendered tend to not be perfectly related due to framerate fluctuations. Do note that this does not make lerp entirely framerate independent.
...
Coming soon: A part 2 where we explore why lerping iteratively is not framerate independent even if $$d$$ is a multiple of $$\Delta time$$, and how we can make it so.
